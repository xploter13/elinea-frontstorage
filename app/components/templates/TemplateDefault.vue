    ...
    <main>
      <section class="market-hero">
        ...
        <div class="market-hero-visual">
          <div v-if="heroProduct" class="market-hero-card">
            <img v-if="productImage(heroProduct)" :src="productImage(heroProduct)!" :alt="heroProduct.name"
                 fetchpriority="high">
            <div v-else class="market-photo-placeholder" aria-hidden="true"><span>{{
      ...
      <section v-if="categories.length" class="market-categories" aria-labelledby="market-categories-title">
        ...
        <div class="market-category-row">
          <NuxtLink v-for="(category, index) in categories" :key="category.id" :to="`/categoria/${category.slug}`"
      ...
      <section class="market-products" aria-labelledby="market-products-title">
        <div class="market-section-heading">
          <div><p>Da nossa vitrine</p>
            <h2 id="market-products-title">Destaques da semana</h2></div>
        ...
        <div v-if="products.length" class="market-product-grid">
          <article v-for="product in products" :key="product.id" class="market-product">
            <NuxtLink :to="`/produto/${product.slug}`" class="market-product-image">
              ...
              <div v-else class="market-photo-placeholder" aria-hidden="true"><span>{{ product.name.charAt(0) }}</span>
              </div>
            ...
            <div class="market-product-info">
              ...
              <div><span v-if="product.original_price">{{
                  money(product.original_price)
                }}</span><strong>{{ money(product.price) }}</strong></div>
            </div>
          </article>
        </div>
...
.market-nav a, .market-section-heading > a {
  color: var(--ink);
  font-size: 14px;
  font-weight: 700;
  text-decoration: none
...
.market-kicker, .market-section-heading p {
  margin: 0 0 18px;
  color: var(--green);
  font-size: 12px;
  font-weight: 900;
...
.market-hero-card > .market-photo-placeholder {
  min-height: 520px
}
...
.market-hero-product strong {
  margin: 7px 0 18px;
  font-size: 20px
}

.market-hero-product b {
  color: var(--green);
  font-size: 18px
}
...
.market-photo-placeholder {
  ...
  height: 100%;
  min-height: 260px;
  background: linear-gradient(145deg, #dff56b, #a9d986);
  color: var(--ink)
}
...
.market-category-row {
  ...
  border-bottom: 1px solid #16352a38
}
...
.market-category {
  ...
  min-height: 140px;
  padding: 20px 14px;
  border-right: 1px solid #16352a38;
  color: var(--ink);
  font-size: 17px;
  font-weight: 800;
  text-decoration: none;
...
.market-product-info > div {
  display: grid;
  justify-items: end;
  flex-shrink: 0
}

.market-product-info span {
  color: #728078;
  font-size: 12px;
...
@media (max-width: 560px) {
  .market-header, .market-hero, .market-categories, .market-products {
    padding-left: 18px;
    padding-right: 18px
  }
...
@media (prefers-reduced-motion: reduce) {
  * {
  ...
  .market-primary, .market-product-image img {
